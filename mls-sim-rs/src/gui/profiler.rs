...
impl GuiApp {
    pub(crate) fn profiler_tab(&mut self, ctx: &egui::Context) {
                ...
                if let Some(ref rid) = room_id {
                    let (available, running, data) = {
                        ...
                        if let Some(r) = mgr.rooms.get(rid) {
                            ...
                            (
                                s.profiler_available,
                                s.profiler_running,
                                s.profile_data.clone(),
                            )
                        } else {
                            (false, false, None)
                        }
                    ...
                    if available {
                        ...
                        ui.horizontal(|ui| {
                            if running {
                                ...
                                ui.label(
                                    egui::RichText::new("●")
                                        .color(egui::Color32::from_rgb(r, g, g)),
                                );
        ...
        let has_data = profile_data.as_ref().map_or(false, |d| d.total_samples > 0);
                                ...
                                .show(ui, |ui| {
                                    ...
                                    for f in funcs.iter().take(30) {
                                        ...
                                        pct_bar(
                                            ui,
                                            self_pct,
                                            egui::Color32::from_rgb(230, 115, 60),
                                        );
                                        ui.label(
        ...
        egui::CentralPanel::default()
            .frame(egui::Frame::central_panel(&ctx.style()).inner_margin(egui::Margin::same(10.0)))
            .show(ctx, |ui| {
...
fn draw_flame_graph(ui: &mut egui::Ui, data: &ProfileData, frame_ms: f64, hover_info: &mut String) {
    let root = &data.root;
    if root.count == 0 {
        return;
    }
...
fn draw_node(
    ...
    if is_root {
        ...
        if w > 32.0 {
            ...
            let clip =
                egui::Rect::from_min_size(egui::pos2(x + 2.0, y), egui::vec2(w - 4.0, BAR_H));
    ...
    for child in &node.children {
        ...
        if cw >= MIN_BAR_W as f64 {
            draw_node(
                painter, child, cx, child_y, cw as f32, root_count, frame_ms, hover_pos,
                hover_info, false,
            );
            cx += cw as f32;
        }
